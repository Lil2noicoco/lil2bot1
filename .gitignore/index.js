...
client.on("ready", () => {
    ...
    client.user.setGame("🌎Aider Des Gens *aide🌎");
    client.channels.get("500994125311639554").send("___**Le bot est connecté, pret à vous aider !**___");
...
client.on('message', async message => { 
    ...
    if(message.content === prefix + "aide") {
      ...
      .addField(":sound: Musique", "Fais `*mus` pour voir mes commandes pour écouter de la musique !")
    ...
    if(message.content === prefix + "mod") {
      ...
      .addField("*clear nombre", "Supprime le nombre de messages indiqué (Nb <100 Ou =100)")
      .addField("*mute <@user>", "Mute l'utilisateur mentionné")
      .addField("*unmute <@user>", "Unmute l'utilisateur mentionné")
      .addField("*warn <@user> <raison>", "Avertir un utilisateur mentionné")
      .addField("*seewarns <@user>", "Voir les avertissements de l'utilisateur mentionné")
      .addField("*deletewarns <@user> <nombre>", "Suprime les avertissements de l'utilisateur mentionné")
  ...
  if(!message.guild.voiceConnection) message.member.voiceChannel.join().then(function(connection) {
    // On peux catch l'erreur que la vidéo ne se trouve pas ducoup il faut *stop le bot.
...
client.on('guildMemberAdd', member => {
  member.createDM().then(channel => {
    return channel.send('Bienvenue sur notre serveur discord ! Faites *Membres dans le salon Verification-account pour avoir accès au serveur entier !' + member.displayName)
  }).catch(console.error)
...
if(msg.content === prefix + "Membres") {
  ...
  msg.reply('Voilà le rôle Membres vous a été atribué !')
  msg.author.createDM().then(channel => {
    channel.send('Tu as vérifié ton compte avec succes, passe un bon moment sur notre serveur Discord !')
  })
  console.log("Rôle Membres atribué")
}
})
